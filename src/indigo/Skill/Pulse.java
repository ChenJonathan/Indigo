...
public class Pulse extends Skill
{
	...
	public void update()
	{
		...
		if(input.mouseLeftDown())
		{
			if(castTime == 0)
			{
				double scale = Math.sqrt(Math.pow(playState.getMouseY() - player.getY(), 2) + Math.pow(playState.getMouseX() - player.getX(), 2));
				...
				playState.getProjectiles().add(new PulseShot(player, player.getX(), player.getY(), velX, velY, PulseShot.DAMAGE));
